class UserController {
    ...
    create(req,res,next) {
        req.body.password = passworkToHash(req.body.password)
        UserService.insert(req.body).then(creat => {
            try {
                evetEmitter.emit("send_create_email", {
                    to: create.email,
    ...
    login(req,res,next) {
        ...
        console.log(email.macth(emailexp))
            ...
            UserService.findOne(req.body).then((user) => {
                ...
                user = {
                    ...
                    tokens: {
                        ...
                        refrest_token: generateRefreshToken(user)
                    }
                }
    ...
    resetPassword(req,res){
        const new_password = `${uuid.v4()?.split("-")[0]}${uuid.v4()?.split("-"[1])}` || `murtaflix-${new Date().getTime()}`
        ...
        UserService.changePassword({email: req.body.email}, {password: passwordToHash (new_password)}).then((updatedPassword) => {
            ...
            evetEmitter.emit("send_email", {
                to: updatedPassword.email,
                subject: "Reset password ✔",

                html: `Hello ${updatedPassword.full_name} ${updatedPassword.last_name} Your Password reset has been successfully completed.. <br /> Don't forget to change your password after logging in.. <br /> New password: <b>${new_password}</b> <h1> MurtaFlix </h1>`
            },
            )
            res.status(hs.OK).send({
                message: "Password change completed succesfully",
                updatedPassword
            })
        }).catch((e) => {
